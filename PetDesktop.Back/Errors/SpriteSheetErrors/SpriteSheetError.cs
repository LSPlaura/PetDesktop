...
public abstract record SpriteSheetError(string Message) : DomainError(Message)
{
    ...
    public sealed record SpriteSheetDeleteError(string Message) : SpriteSheetError(Message);
    public sealed record SpriteSheetFindError(string petName, string spriteSheetName) : 
        SpriteSheetError($"SpriteSheet vinculed to the pet {petName} and named {spriteSheetName} couldnt be found");
}